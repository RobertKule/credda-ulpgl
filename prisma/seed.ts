...
import { Role, Domain, MediaType } from '@prisma/client';
...
async function generateUniqueSlug(title: string, counter: number = 0): Promise<string> {
  const baseSlug = generateSlug(title);
  const slug = counter > 0 ? `${baseSlug}-${counter}` : baseSlug;
  
  // ✅ CORRIGÉ: enlever $executeRaw
  const existing = await db.publication.findUnique({
    where: { slug }
  });
  
  if (existing) {
    return generateUniqueSlug(title, counter + 1);
  }
...
async function main() {
  ...
  const robertPassword = await bcrypt.hash('credda@2026', 12);

  // Admin principal
  const admin = await db.user.create({  // ✅ CORRIGÉ
    data: {
  ...
  // Éditeur
  const editor = await db.user.create({  // ✅ CORRIGÉ
    data: {
  ...
  // Chercheur
  const researcher = await db.user.create({  // ✅ CORRIGÉ
    data: {
  ...
  // Admin Robert
  const robert = await db.user.create({  // ✅ CORRIGÉ
    data: {
      email: 'kulewakangitsirobert@gmail.com',
      password: robertPassword,
      name: 'Robert KULE',
      role: Role.ADMIN,
    },
  });
  console.log('✅ Admin Robert créé:', robert.email);
  ...
  for (const cat of categories) {
    const category = await db.category.create({  // ✅ CORRIGÉ
      data: {
        slug: cat.slug,
        translations: {
          create: [
...
L'articulation entre justice formelle et informelle constitue une piste prometteuse pour une justice transitionnelle contextualisée et inclusive.`,
  ...
  for (const article of researchArticles) {
    ...
    await db.article.create({  // ✅ CORRIGÉ
      data: {
        slug: article.slug,
...
## Introduction
La République Démocratique du Congo dispose d'un sous-sol extrêmement riche, mais cette richesse ne profite pas suffisamment aux communautés locales. Cette étude examine les mécanismes de redistribution des revenus miniers et leur impact sur le développement local.

## Méthodologie
Notre recherche s'appuie sur une enquête de terrain menée dans 12 sites miniers du Nord-Kivu entre janvier et décembre 2024, complétée par une analyse documentaire des cadres légal et réglementaire.
...
## Introduction
The Democratic Republic of Congo has an extremely rich subsoil, but this wealth does not sufficiently benefit local communities. This study examines mining revenue redistribution mechanisms and their impact on local development.

## Methodology
Our research is based on field surveys conducted in 12 mining sites in North Kivu between January and December 2024, complemented by documentary analysis of the legal and regulatory frameworks.
...
Reform of the Congolese mining sector necessarily requires better revenue redistribution and effective participation of local communities in decisions affecting them.`,
  ...
  for (const pub of publications) {
    const slug = await generateUniqueSlug(pub.translations[0].title);
    
    await db.publication.create({  // ✅ CORRIGÉ
      data: {
        slug: slug,
        year: pub.year,
        doi: pub.doi,
    ...
    console.log(`✅ Publication: ${pub.translations[0].title} (slug: ${slug})`);
  }
  ...
  for (const member of members) {
    await db.member.create({  // ✅ CORRIGÉ
      data: {
        order: member.order,
        email: member.email,
        image: member.image,
...
Pr. Kennedy Kihangi Bindu`,
  ...
  for (const msg of contactMessages) {
    await db.contactMessage.create({  // ✅ CORRIGÉ
      data: {
        name: msg.name,
        email: msg.email,
        subject: msg.subject,
  ...
  await db.media.createMany({  // ✅ CORRIGÉ
    data: [
      {
  ...
  console.log('\n📊 RÉCAPITULATIF:');
  console.log(`   - ${await db.user.count()} utilisateurs`);  // ✅ CORRIGÉ
  console.log(`   - ${await db.category.count()} catégories`);  // ✅ CORRIGÉ
  console.log(`   - ${await db.article.count()} articles`);  // ✅ CORRIGÉ
  console.log(`   - ${await db.publication.count()} publications`);  // ✅ CORRIGÉ
  console.log(`   - ${await db.member.count()} membres d'équipe`);  // ✅ CORRIGÉ
  console.log(`   - ${await db.contactMessage.count()} messages`);  // ✅ CORRIGÉ
  console.log(`   - ${await db.media.count()} médias`);  // ✅ CORRIGÉ
  console.log('\n' + '='.repeat(50));
  console.log('\n🔐 IDENTIFIANTS DE CONNEXION :');
  console.log('   Admin 1   : admin@credda-ulpgl.org / Admin123!');
  console.log('   Admin 2   : kulewakangitsirobert@gmail.com / credda@2026');
