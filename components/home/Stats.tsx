...
export default function Stats({ years, totalResources, partners, clinicalCases = 0 }: StatsProps) {
  ...
  return (
    <section className="relative overflow-hidden bg-card shadow-[0_4px_24px_-4px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_24px_-4px_rgba(0,0,0,0.3)] py-12 sm:py-16 lg:py-24">
      <div className="w-full px-5 sm:px-8 lg:px-12 xl:px-16 bg-foreground">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-0">
          {stats.map((stat, idx) => (
            <GSAPReveal key={idx} direction="up" delay={idx * 0.1}>
              <div className="relative flex flex-col items-center text-center lg:border-r last:border-0 border-primary/10 px-10 group">
                <div className="mb-8 p-6 bg-primary/5 dark:bg-primary/10 border border-primary/15 group-hover:bg-primary group-hover:border-primary transition-all duration-700 relative overflow-hidden rounded-2xl">
