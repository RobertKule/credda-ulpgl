// app/[locale]/login/page.tsx - REMPLACER CE FICHIER
"use client";
...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ API de login simple (sans next-auth pour l'instant)
      ...
      <div className="hidden md:flex md:w-1/2 bg-[#050a15] relative overflow-hidden items-center justify-center p-12">
        ...
        <div className="relative z-10 max-w-md text-center md:text-left space-y-8">
          ...
          <p className="text-slate-400 font-light leading-relaxed">
            ...
            aux chercheurs et administrateurs autorisés.
          </p>
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12 lg:p-24 bg-slate-50">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="w-full max-w-sm space-y-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {error && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="p-3 bg-red-50 border-l-4 border-red-500 text-red-700 text-xs font-medium">
                {error}
              </motion.div>
            )}
            ...
            <Button type="submit" disabled={isLoading} className="w-full h-14 bg-slate-900 hover:bg-blue-600 text-white rounded-none font-bold uppercase tracking-[0.2em] transition-all group shadow-xl shadow-slate-200">
              {isLoading ? (
