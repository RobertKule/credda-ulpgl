...
export default async function TrackCasePage({ 
  ...
  const statusColors: any = {
    ...
    ACTION_ENGAGED: "bg-secondary/20 text-secondary",
    RESOLVED: "bg-emerald-100 text-emerald-700",
    CLOSED: "bg-slate-100 text-slate-500",
  };

  return (
    <main className="min-h-screen bg-soft-cream/30 pt-32 pb-48">
      <div className="container mx-auto px-6 max-w-4xl">
        
        {/* Header */}
        <div className="text-center space-y-8 mb-20">
           <div className="inline-flex items-center gap-3 text-secondary text-[10px] font-black uppercase tracking-[0.4em]">
              <ShieldCheck size={20} />
              <span>{t('badge')}</span>
           </div>
        ...
        {/* Search Bar */}
        <div className="bg-white border border-light-gray p-2 shadow-3xl mb-20 flex flex-col md:flex-row gap-2">
           <form action="" className="flex-1 flex flex-col md:flex-row gap-2">
              <div className="flex-1 relative">
                 <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-light-gray" size={20} />
                 <Input 
                   name="phone" 
                   defaultValue={phone}
        ...
        {searched && (
          <div className="space-y-12">
            <div className="flex items-center justify-between border-b border-light-gray pb-6">
               <h3 className="text-2xl font-serif font-black text-primary">{t('results_title')} ({cases.length})</h3>
               <History size={20} className="text-light-gray" />
            </div>

            {cases.length > 0 ? (
              <div className="grid gap-6">
                {cases.map((c) => (
                  <div key={c.id} className="bg-white border border-light-gray p-10 hover:border-secondary transition-all group">
                     <div className="flex flex-col md:flex-row justify-between items-start gap-8">
                        <div className="space-y-4 flex-1">
                           ...
                           <h4 className="text-2xl font-serif font-black text-primary group-hover:text-secondary transition-colors">{c.title}</h4>
                        ...
                        <div className="md:text-right space-y-2">
                           ...
                           <div className="flex items-center md:justify-end gap-2 text-primary font-black text-xs uppercase">
                              <Clock size={14} className="text-secondary" />
                              {new Date(c.updatedAt).toLocaleDateString()}
                           </div>
                        </div>
                     </div>
                     <div className="mt-10 pt-8 border-t border-soft-cream flex justify-between items-center">
                        <Link href={`#`} className="text-[9px] font-black uppercase tracking-widest text-primary flex items-center gap-2 group/link opacity-50 cursor-not-allowed">
                          View Details <ChevronRight size={14} className="group-hover/link:translate-x-1 transition-transform" />
                        </Link>
                        <Gavel size={16} className="text-light-gray" />
                     </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="py-24 text-center bg-white border border-light-gray space-y-6">
                 <AlertCircle size={48} className="mx-auto text-light-gray" strokeWidth={1} />
                 <h4 className="text-2xl font-serif font-black text-primary">{t('no_records')}</h4>
        ...
        {!searched && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-20">
             <div className="p-10 border border-light-gray bg-white space-y-6">
                <div className="p-4 bg-secondary/10 text-secondary w-fit">
                   <ShieldCheck size={28} />
                </div>
                <h4 className="font-serif font-black text-primary uppercase text-xs tracking-widest">{t('confidentiality_title')}</h4>
             ...
             <div className="p-10 border border-light-gray bg-white space-y-6">
                ...
                <h4 className="font-serif font-black text-primary uppercase text-xs tracking-widest">{t('authority_title')}</h4>
