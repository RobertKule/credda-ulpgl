      ...
      <div className="bg-white border border-slate-200 p-10 shadow-2xl space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <Input required value={data.name} onChange={e => setData({...data, name: e.target.value})} className="rounded-md" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-widest">Email Professionnel</label>
            <Input required type="email" value={data.email} onChange={e => setData({...data, email: e.target.value})} className="rounded-md" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-widest">Mot de passe provisoire</label>
            <Input required type="password" value={data.password} onChange={e => setData({...data, password: e.target.value})} className="rounded-md" />
          </div>
          <Button type="submit" className="w-full bg-primary rounded-md h-14 uppercase font-black text-xs tracking-widest">
            Générer l'accès
          </Button>
        </form>
      </div>
