...
import { FileText, Download, Filter, SearchX, BookOpen, User2, Calendar } from "lucide-react";
                ...
                <div key={pub.id} className="bg-white border border-slate-200 flex flex-col md:flex-row group hover:shadow-2xl transition-all duration-500">
                  ...
                  <div className="p-8 flex flex-col justify-between flex-1">
                    <div className="space-y-4">
                      ...
                      <h2 className="text-2xl font-serif font-bold text-slate-900 leading-tight group-hover:text-blue-700 transition-colors">{content.title}</h2>
                    ...
                    <div className="pt-6 mt-6 border-t border-slate-50 flex items-center justify-between">
                      <a href={pub.pdfUrl} target="_blank" className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-blue-900 hover:gap-4 transition-all">
                        <Download size={14} /> Télécharger (PDF)
                      </a>
                      {pub.doi && <span className="text-[9px] font-mono text-slate-300">DOI: {pub.doi}</span>}
                    </div>
                  </div>
                </div>
              );
